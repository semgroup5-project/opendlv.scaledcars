...
String decodedNetstring(String netstring) {
    if (netstring.length() < 3)
        return "error"; //if the Netstring is less than 3 characters, it's either an invalid one or contains an empty string
    ...
    String parsedDigits = netstring.substring(0,
                                              semicolonIndex); //parse until the semicolon. Those should be the control digits
    int controlDigits = parsedDigits.toInt();
    if (controlDigits < 1)
        return "error"; //if the control digit is smaller than 1, then it's either not a digit or the Netstring is invalid
    ...
    if (command.substring(command.length() - 1) == ",")
        command.remove(command.length() - 1); //if last character is a comma, remove it
    if (command.length() != controlDigits)
        return "error"; //if string's length isn't equal with the control digits, it's an invalid Netstring
    return command;
}
